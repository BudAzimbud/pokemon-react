...
export const getAllPokemon = createAsyncThunk("pokemons/list",async({limit})=>{
    const getLinkPokemon = await axios.get(`https://pokeapi.co/api/v2/pokemon?limit=${limit}`)
    const promisePokemon = await getLinkPokemon.data.results.map(async (pokemon)=>{
...
export const findPokemon = createAsyncThunk("pokemons/find",async({name})=>{
